...
class Paypal {
   ...
   */
   function __construct() {
      ...
      $this->ipn_log_file = base_path('logs/ipn_results.log');
      $this->ipn_log = TRUE;
      $this->ipn_response = '';
   ...
   */
    ...
    * is redirected to paypal.
    */
   function get_submit_paypal_post() {

     $result = '';
     $result .= "<form method=\"post\" name=\"paypal_form\" ";
     $result .= "action=\"".$this->paypal_url."\">\n";

     foreach ($this->fields as $name => $value) {
       $result .= "<input type=\"hidden\" name=\"$name\" value=\"$value\"/>\n";
     }
     //$result .= "<p><label class=\"labeltab\">If you are not automatically redirected to paypal within 5 seconds...</label></p>\n";
     //$result .= '<span class="btn blue"><strong><input type="submit" value="Click Here" /></strong></span>';
     //$result .= "\n";
   ...
   function validate_ipn() {
      ...
      //log_message('debug', '$url_parsed: '.print_r($url_parsed, true));
      //log_message('debug', 'validate_ipn $post_string: '.$post_string);

      // open the connection to paypal
      $ipn_url = 'ssl://'.$url_parsed['host'];
      $ipn_port = 443;
      $fp = fsockopen($ipn_url, $ipn_port, $err_num, $err_str, 30);
      if(!$fp) {
